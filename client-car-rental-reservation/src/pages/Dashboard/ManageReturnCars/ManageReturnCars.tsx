...
import { useCarReturnAndUpdateDateMutation, useGetAllBookingsQuery } from "../../../redux/baseApi";
...
const ManageReturnCars: React.FC = () => {
  ...
  const handleEditClick = async(id: string, currentEndTime: string) => {
    ...
    const res = await updateReturnCar(updateTime).unwrap();
    console.log(res);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            {isLoading && <div>Loading...</div>}
              { data?.data && data?.data?.map((booking: TBooking) => (
                <tr key={booking._id}>
                  ...
                  <td className="py-4 px-6 text-sm flex gap-2">
                    {editBookingId === booking._id ? (
                      <>
                        <button
                          onClick={() => handleUpdateEndTime(booking._id as string)}
                      ...
                      <button
                        onClick={() => handleEditClick(booking._id as string, booking?.endTime)}
